...
<body>
		...
		<a href="/impersonation/revert" class="impersonate"><div class="alert-info text-center">Click Here End Account Impersonation</div></a>
		@endif
	...
	<?php if(isset($checkpointDate)){ ?>
    <?php $checkpointTime = date("m/d/y H:i:s",strtotime($checkpointDate)); ?>
    <script type="text/javascript">		
		var ccountDownDate = new Date('<?php echo $checkpointTime; ?>').getTime();
		var cx = setInterval(function(){
			...
			$("#checkpoint-countdown-holder").html("<div class='days'>" + cdays + "<span>days</span></div><div class='hours'>" + chours + "<span>hours</span></div><div class='minutes'>" + cminutes + "<span>minutes</span></div><div class='seconds'>" + cseconds + "<span>seconds</span></div>");
		}, 1000);
	</script>
	<?php } ?>
</body>
